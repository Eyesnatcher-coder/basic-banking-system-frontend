...
const GridcustomerList = () => {
    ...
    return (
        <div>
            ...
            <div style={{ backgroundColor: "#FFFFFF10",margin:"5px", padding: "0.5%",borderRadius:"100px",display:"flex",flexWrap:"wrap",justifyContent:"center" }}>
                ...
                {
                    users.map((user) => {
                        return <CustomerGrid name={user.name} account={user.account} email={user.email} savings={user.savings} />
                    })}
            </div>
        </div>
    );
